...
void server::start_accept(){
	boost::shared_ptr<ipc_connection> new_connection = ipc_connection::create(acceptor.get_io_service());
  
  	acceptor.async_accept(new_connection->socket(), boost::bind(&server::handle_accept, this, new_connection, _1));
 
}
void server::handle_accept(boost::shared_ptr<ipc_connection> new_connection, const boost::system::error_code& error){
  if(!error){
